...
export default {
  async fetch(request, env) {
    ...
    if (request.method === "GET" || request.method === "HEAD") {
      return new Response(
        request.method === "HEAD" ? null : appHtml,
        {
          status: 200,
          headers: {
...
async function renderPreview(request, env) {
  if (!env.AI) {
    return json({
      code: "CONFIG_MISSING",
      message: "Workers AI binding 'AI' is not configured."
    }, 503);
  }
...
- Render individual natural hair strands, realistic density, follicles, authentic edges and lighting.
...
- The result must look like a real professional barber preview photograph of this exact person wearing ${styleName}.`;
}
